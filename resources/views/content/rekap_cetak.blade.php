...
<body>
    <div class="content">
        <div class="header">
            <h1><strong>Rekap Transaksi</strong></h1>
            ...
            <table class="table table-bordered">
                <thead>
                    <tr style="text-align: center">
                        ...
                        <th class=" no-border-right" scope="col">Jumlah Terjual</th>
                        <th class=" no-border-right" scope="col">Arus</th>
                        <th class=" no-border-right" colspan="2" scope="col">Total</th>
                    </tr>
                </thead>
                <tbody>
                    {{-- {{ dd($totalpendapatan) }} --}}
                    @foreach ($totalpendapatan as $item)
                        <tr style="text-align: center">
                            <th class=" no-border-right" scope="row">{{ $loop->iteration }}</th>
                            <td class=" no-border-right">{{ $item->obat->nama_obat }}</td>
                            <td class=" no-border-right">{{ $item->jumlah }}</td>
                            <td class=" no-border-right">{{ $item->jenis}}</td>
                            <td class=" no-border-right">Rp. </td>
                            <td class="uang no-border-right">{{ number_format($item->harga, 0, ',', '.') }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
        ...
        <div class="total">
            <p><strong>Total Pendapatan: Rp. {{ number_format($totalSeluruhPendapatan, 0, ',', '.') }}</strong></p>
        </div>
